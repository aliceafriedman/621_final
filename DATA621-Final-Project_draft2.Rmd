  ...
  df_por%>% 
    ...
    rowwise() %>%
      mutate(
        ...
        missed_exam = if_else(G1_por==0 | G2_por==0 | G1_mat==0 | G2_mat==0, 1, 0)
      )
...
df_both$schoolsup <- binary(df_both, "schoolsup") %>% as.numeric()
...
```{r remove}
df_both <- df_both %>% filter(missed_exam == 0)
```
...
p7 <- df_both %>% ggplot( aes(factor(dayAlc), final_grade)) + geom_boxplot() + xlab("Daily Alc")
p8 <- df_both %>% ggplot( aes(factor(weekAlc), final_grade)) + geom_boxplot() + xlab("Weekly Alc")
...
We will first attempt the simplest case multiple linear regression using the full subset method to choose the best linear model. Using the `leaps` package and 3-fold cross validation from the `caret` package, we can select the best performing model among possile linear regression strategies.
...
for(i in 1:3){
  ...
  bic <- summary(bestsub)$bic
  plot(1:8, bic) + 
    abline(h= mean(bic)-sd(bic))
}
```
...
for (i in 1:3){
  cors <- c(cors, runXval(i)[[1]])
  r.sq <- c(r.sq, runXval(i)[[2]])
}
...
```{r final_failures}
df_both %>% 
  ggplot(aes(y=final_grade, x=factor(parentEdu), fill=factor(failures))) + 
  facet_grid(~sex_F)+
  geom_boxplot() + 
  xlab("Parental Education") + 
  ylab("Final Grade") + 
  ggtitle("Final Grade by Previously Failed")

stats::aov(final_grade ~ failures + sex_F + parentEdu, df_both) %>%
summary()
...
data_step = stepAIC(lm, scope = list(upper = ~ .^2, lower = ~1))
summary(data_step)

# Predict results to evaluate and validate

yhat_step <- predict(data_step, test_data)
cor(yhat_step, test_data$final_grade)
rmse(test_data$final_grade, yhat_step)
rmse(test_data$final_grade, predict(lm, test_data))
```
...
Random Forrest has superior test results, with a correlation between predicted and actual results of 0.57 and a lower RMSE of 3.79.

```{r echo=FALSE}

data_rf <- randomForest(
  final_grade ~ traveltime + studytime + failures + schoolsup + famEdsup + paid_tutor + activities +	nursery +	higher + internet +	romantic + famrel +	freetime + friendtime +	dayAlc +	weekAlc +	health +	absences_mean + parentEdu, 
                        data = train_data)
...
yhat_rf <- predict(data_rf, newdata = test_data)
...
In a complex model, a decision tree can be effective. This model results in correlation between predicted and actual final grades of 0.52 as well as an RMSE of 3.86. 

```{r echo=FALSE}
data_tr <- rpart(final_grade ~ traveltime + studytime + failures + schoolsup + famEdsup + paid_tutor + activities +	nursery +	higher + internet +	romantic + famrel +	freetime + friendtime +	dayAlc +	weekAlc +	health +	absences_mean + parentEdu, data = train_data)
...
boxplot(list(lm = y_obs-yhat_step,
             rf = y_obs-yhat_rf,
             tr = y_obs - yhat_tr), ylab="Error in Validation Set")

abline(h=0, lty=2, col='blue')

results <- tibble(y_obs, yhat_step, yhat_tr, yhat_rf)

results %>% gather(key, value, -y_obs) %>% ggplot(aes(x=value, y=y_obs)) + geom_point() + facet_grid(~key)
```
We can further compare the results below.
...
The most significant predictor for the students final grade turns out to be having previousy failed a class. This is an interesting and actionable results, as students who have previously failed are a known population to whom extra school support can be given. School support, thankfully, also rises to the top of the predictors.
